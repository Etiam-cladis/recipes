...
public:
    ...
    using const_iterator_adjacent_nodes =
        ConstAdjacentNodesIterator<DirectedGraph>;
    ...
    DirectedGraph(std::initializer_list<T> list,
                  const Allocator &allcator = Allocator{});
    ...
    requires((has_size_type<Index> || std::convertible_to<Index, size_t>)&&std::
                 invocable<Func, Index,
                           Args...>)  //
    ...
    const_reverse_iterator_adjacent_nodes rbegin(
        const T &nodeValue) const noexcept;
    const_reverse_iterator_adjacent_nodes rend(
        const T &nodeValue) const noexcept;
    const_reverse_iterator_adjacent_nodes crbegin(
        const T &nodeValue) const noexcept;
    const_reverse_iterator_adjacent_nodes crend(
        const T &nodeValue) const noexcept;
...
private:
    ...
    using nodesContainerType =
        std::vector<details::DirectedGraphNode<T, Allocator>>;
    nodesContainerType m_nodes;

    Allocator m_allocator;

    [[nodiscard]] typename nodesContainerType::iterator findNode(
        const T &nodeValue);
    [[nodiscard]] typename nodesContainerType::const_iterator findNode(
        const T &nodeValue) const;

    //[[nodiscard]] std::set<T, std::less<>, Allocator>
    // getAdjancentNodesValues(const typename details::DirectedGraphNode<T,
    // Allocator>::adjancncyNodesListType &indicies) const;

    void removeAllLinks(typename nodesContainerType::const_iterator it);

    [[nodiscard]] size_t getIndexOfNode(
        const typename nodesContainerType::const_iterator &node) const noexcept;
};
...
template <typename T, typename Allocator>
DirectedGraph<T, Allocator>::DirectedGraph(std::initializer_list<T> list,
...
template <typename T, typename Allocator>
template <typename Iter>
DirectedGraph<T, Allocator>::DirectedGraph(Iter first, Iter last,
                                           const Allocator &allocator)
    : m_nodes{allocator}, m_allocator{allocator}
{
    assign(first, last);
...
template <typename T, typename Allocator>
DirectedGraph<T, Allocator> &DirectedGraph<T, Allocator>::operator=(
...
{
    clear();
    for (auto beg{first}; beg != last; ++beg) { insert(*beg); }
}
...
DirectedGraph<T, Allocator>::insert(T &&value)
{
    ...
    return {
        iterator{std::prev(std::end(m_nodes)), *this},
        true
    };
}
...
template <typename T, typename Allocator>
std::pair<typename DirectedGraph<T, Allocator>::iterator, bool>
DirectedGraph<T, Allocator>::insert(
    typename DirectedGraph<T, Allocator>::const_iterator pos, const T &value)
{
    return insert(value);
}
...
std::pair<typename DirectedGraph<T, Allocator>::iterator, bool>
DirectedGraph<T, Allocator>::insert(
    typename DirectedGraph<T, Allocator>::const_iterator pos, T &&value)
{
...
DirectedGraph<T, Allocator>::erase(
    typename DirectedGraph<T, Allocator>::const_iterator pos)
{
...
typename DirectedGraph<T, Allocator>::iterator
DirectedGraph<T, Allocator>::erase(
    typename DirectedGraph<T, Allocator>::const_iterator first,
...
template <typename T, typename Allocator>
void DirectedGraph<T, Allocator>::updateNodeValue(
    typename DirectedGraph<T, Allocator>::iterator pos, const T &newValue)
{
    auto temp{newValue};
...
template <typename T, typename Allocator>
void DirectedGraph<T, Allocator>::updateNodeValue(
    typename DirectedGraph<T, Allocator>::iterator pos, T &&newValue)
{
    (*pos).value() = std::move(newValue);
}

template <typename T, typename Allocator>
bool DirectedGraph<T, Allocator>::insertEdge(const T &fromNodeValue,
                                             const T &toNodeValue)
{
...
template <typename T, typename Allocator>
bool DirectedGraph<T, Allocator>::eraseEdge(const T &fromNodeValue,
                                            const T &toNodeValue)
{
    auto from{findNode(fromNodeValue)};
    auto to{findNode(toNodeValue)};
...
DirectedGraph<T, Allocator>::operator[](
    typename DirectedGraph<T, Allocator>::size_type index)
{
...
DirectedGraph<T, Allocator>::operator[](
    typename DirectedGraph<T, Allocator>::size_type index) const
{
    return m_nodes[index].value();
}

template <typename T, typename Allocator>
typename DirectedGraph<T, Allocator>::reference DirectedGraph<T, Allocator>::at(
    typename DirectedGraph<T, Allocator>::size_type index)
{
...
template <typename Index, typename Func, typename... Args>
requires(
    (has_size_type<Index> || std::convertible_to<Index, std::size_t>)&&std::
        invocable<Func, Index,
                  Args...>)  //
    void DirectedGraph<T, Allocator>::dfs(Index startIndex, Func &&fuc,
                                          Args &&...args)
{
    ...
    while (!stk.empty())
    {
        ...
        stk.pop();
        std::invoke(std::forward<Func>(fuc), topIndex,
                    std::forward<Args>(args)...);

        const std::set<std::size_t> &topAdjancncyNodeList{
            m_nodes[topIndex].getAdjancncyNodeList()};
        std::for_each(std::begin(topAdjancncyNodeList),
                      std::end(topAdjancncyNodeList), [&stk, &visted](Index i) {
                          if (visted[i] == 0)
                          {
                              visted[i] = 1;
                              stk.push(i);
...
requires((has_size_type<Index> || std::convertible_to<Index, size_t>)&&std::
             invocable<Func, Index,
                       Args...>)  //
    void DirectedGraph<T, Allocator>::bfs(Index startIndex, Func &&fuc,
                                          Args &&...args)
{
    ...
    while (!que.empty())
    {
        ...
        const std::set<std::size_t> &topAdjancncyNodeList{
            m_nodes[topIndex].getAdjancncyNodeList()};
        std::for_each(std::begin(topAdjancncyNodeList),
                      std::end(topAdjancncyNodeList), [&que, &visted](Index i) {
                          if (visted[i] == 0)
                          {
                              visted[i] = 1;
                              que.push(i);
...
DirectedGraph<T, Allocator>::begin(const T &nodeValue) noexcept
{
    ...
    if (iter != std::end(m_nodes))
    {
        return iterator_adjacent_nodes{std::begin(iter->getAdjancncyNodeList()),
                                       *this};
    }
...
DirectedGraph<T, Allocator>::end(const T &nodeValue) noexcept
{
    ...
    if (iter != std::end(m_nodes))
    {
        return iterator_adjacent_nodes{std::end(iter->getAdjancncyNodeList()),
                                       *this};
    }
...
DirectedGraph<T, Allocator>::begin(const T &nodeValue) const noexcept
{
    ...
    if (iter != std::end(m_nodes))
    {
        return const_iterator_adjacent_nodes{
            std::begin(iter->getAdjancncyNodeList()), *this};
    }
...
DirectedGraph<T, Allocator>::end(const T &nodeValue) const noexcept
{
    ...
    {
        return const_iterator_adjacent_nodes{
            std::end(iter->getAdjancncyNodeList()), *this};
    }
...
DirectedGraph<T, Allocator>::findNode(const T &nodeValue)
{
    return std::find_if(
        ...
        [&nodeValue](const auto &value) { return nodeValue == value.value(); });
}
...
template <typename T, typename Allocator>
void DirectedGraph<T, Allocator>::removeAllLinks(
    typename nodesContainerType::const_iterator it)
{
...
template <typename T, typename Allocator>
size_t DirectedGraph<T, Allocator>::getIndexOfNode(
    const typename nodesContainerType::const_iterator &node) const noexcept
{
